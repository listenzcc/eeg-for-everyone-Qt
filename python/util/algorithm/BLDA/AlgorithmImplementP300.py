...
class AlgorithmImplementP300():
    ...
    def __init__(self, path):
        ...
        self.sub_name = time.strftime('%Y_%m_%d_%H_%M', time.localtime(time.time()))
    ...
    def run(self, raw_data, sub_name):
        ...
        y_prob = self.method.test(x)  # 用训练完的 BLDA 进行测试，y_prob (stims,1), 其为 BLDA 预测的概率值，未做归一化

        # 在训练数据上以非叠加平均方式计算字符准确率
        char_acc = cal_char_acc(StimulationConfig, y_prob, af_format_data['evt'][:, 1:3])
        print(f"character accuracy is {char_acc:.5f} (without round average).\n")

        # 在训练数据上以叠加平均方式计算字符准确率
        y_prob, evt_tar = round_average(StimulationConfig, y_prob, af_format_data['evt'][:, 1:3])
        ...
        # 保存最好的模型
        self.method.load(results['model_list'][results['best_index']])  # 加载交叉验证中最好的模型
    ...
    def __data_formatting(self, data, verbose=True):
        ...
        raw_data = data['data']
        raw_evt = data['evt'][(data['evt'][:, 1] < 200), :] # 因为是训练，这句话里滤掉了很多标签
        # 滤波，包括带通滤波和陷波滤波
        tmp0_data = self.__bandpass_notch_filter(raw_data)

        # 重参考 T7 T8
        if len(self.re_samp) > 0:
            idx_re_samp, idx_non_re_samp = channel_name_to_index(data['ch_name'], self.re_samp)  # 记录数据通道中是否包含重参考通道
            tmp_data = tmp0_data - np.tile(np.mean(tmp0_data[idx_re_samp, :], axis=0), (tmp0_data.shape[0], 1))
        else:
            tmp_data = tmp0_data

        # 确定选择通道的索引，这里有点资源冗杂了，其实是可以精简一下的

        self.select_channel, _ = channel_name_to_index(data['ch_name'], self.select_channel_name)

        # 标签处理
        tmp_evt = np.concatenate((raw_evt, np.zeros((raw_evt.shape[0], 1))), axis=1).astype(int) # 在这里加一列0，用来标记是否是目标
        ...
        for i_tar_index in range(len(tar_evt)):
            ...
            tar_loc = find_element(self.stim_target_rule, tar_idx)  # 这边是看一下目标字符是在刺激规则中的哪个位置
            ...
            if i_tar_index == len(tar_evt) - 1:
                ...
                new_evt = tmp_evt[tar_evt[i_tar_index] + 1:tar_evt[i_tar_index + 1], :]
            ...
            for i_stim in range(new_evt.shape[0]):
                tmp1_data = tmp_data[self.select_channel, new_evt[i_stim, 0]:new_evt[i_stim, 0] + self.cal_len]
        ...
        if verbose:
            print(f"Data formatting is finished, there are totally {af_format['data'].shape[2]} stims.\n")
        return af_format
    ...
    def __bandpass_notch_filter(self, data):
        ...
        af_notch_data = signal.filtfilt(b0, a0, data, axis=1)  # 维度的默认值就是-1，其实用默认值也行
        # 带通滤波
        order = 2 # 阶数

...
def gcd_multiple(numbers):
    ...
    for num in numbers[1:]:
        gcd_value = math.gcd(int(gcd_value * 1e3), int(num * 1e3)) / 1e3
    return gcd_value
...
def compute_phase_diff(data, sim_freq, sampling_rate):
    '''
    绘制相位图和相位差图
    p_1, dp = phase_plot(data, type, freq, rfs)
    该函数遍历频率，提取匹配数据并计算其均值执行FFT分析，提取相位信息并计算相邻频率之间的相位差
    ...
    freq_sort = np.sort(freqs)
    freq_index = np.argsort(freqs)
    freq_resolution = gcd_multiple(np.round(np.diff(freq_sort), 10))
    Npoints = round(sampling_rate / freq_resolution)
